...
class Alg:
    ...
    def get_info(self):
        ...
        return:
         a dictionary containing the algorithm parameter outcomes

        description:
         this method returns the information calculated mid processing of the current algorithm
        """

        # get the algorithm parameter outcomes
        #
...
class TRANSFORMER:
    ...
    def train(self, data: MLToolsData,
        ...
        #
        self.is_early_stopping = bool(self.params_d[ALG_NAME_ESTOP])
        self.val_frac = float(self.params_d[ALG_NAME_VALF])
        self.tol = float(self.params_d[ALG_NAME_TOLERANCE])
        self.n_iter_no_change = int(self.params_d[ALG_NAME_MAXITER])
        ...
        n_val = int(self.val_frac * len(samples))
        val_indices = rnd_indices[:n_val]
        train_indices = rnd_indices[n_val:]
        ...
        for epoch in range(self.epochs):
            ...
            #
            rnd_indices_train = np.arange(len(train_samples))
            np.random.shuffle(rnd_indices_train)
            ...
            for batch_start in range(0, len(train_samples), self.batch_size):
                ...
                #
                batch_indices = rnd_indices_train[batch_start:
                                    batch_start + self.batch_size]
                current_samples = train_samples[batch_indices]
                current_labels = train_labels[batch_indices]
            ...
            # 
            self.model_d[ALG_NAME_MDL].eval()
            ...
            # setting torch.no_grad() to disable gradient calculation
            #
            with torch.no_grad():
                # get the output from the model using val_samples
                #
                output = self.model_d[ALG_NAME_MDL](val_samples)
                
                # calculate the loss
                #
                loss = criterion(output,
                                 self.model_d[ALG_NAME_MDL]\
                                 .to_tensor(val_labels))
            ...
            print(f"Epoch: {epoch + 1}/{self.epochs}, "
                  ...
                  f"Validation Loss: {avg_val_loss:{ALG_FMT_DEC}}")
            
            # check for early stopping
            #
            if self.is_early_stopping:
                # if early stopping is enabled, check if the validation loss 
                # has improved
                #
                if best_val_loss - avg_val_loss >= self.tol:
                    best_val_loss = avg_val_loss
                    early_stop_counter = 0
                    
                # if not improved, increase the counter by 1
                #
                else:
                ...
                #
                if early_stop_counter >= self.n_iter_no_change:
